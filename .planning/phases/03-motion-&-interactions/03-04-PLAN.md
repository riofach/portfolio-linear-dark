...
<task type="auto">
  ...
  <action>
    ...
    1. **CRITICAL:** Preserve the existing `<FadeIn>` wrapper added in Plan 02. Do NOT remove it.
    2. Define Zod schema: `name` (min 2), `email` (email), `message` (min 10).
    ...
    4. Add `onSubmit` handler:
       ...
       - Set `isSuccess` state.
       - Reset form.
    5. Bind inputs to `register`.
    6. Show validation errors (red text/border) using `motion` (shake or fade-in).
  </action>
  <verify>
    - Imports `useForm`, `zod`.
    - Handles submit event.
    - Preserves FadeIn wrapper.
  </verify>
